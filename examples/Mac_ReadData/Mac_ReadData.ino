...
void loop() {
  ...
  // WARNING! Reading 128 B works on Arduino Uno, but setting the size of backData to more than ~220 fills the whole SRAM when using Authentication and Mac CommMode
  ...
  // To use this example you must first change the CommMode for a file using the DNA_Full_ChangeFileSettings example.
  file = MFRC522_NTAG424DNA::DNA_FILE_CC;
  lengthToRead = 32;
  /*file = MFRC522_NTAG424DNA::DNA_FILE_NDEF;
  lengthToRead = 128; // set to 256 on ESP8266 when reading NDEF file */
  ...
  byte offset = 0;
  dna_statusCode = ntag.DNA_Mac_ReadData(file, lengthToRead, offset, backData, &backLen);
  if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_OK){
    Serial.print(F("Mac_ReadData STATUS NOT OK: "));
    Serial.println(dna_statusCode);
    if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_TIMEOUT)
      // Errors other than timeout appear often when slowly approaching a card to the reader.
      // When it happens, the card has to be deselected and woken up.
