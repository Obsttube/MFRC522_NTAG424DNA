...
void loop() {
  ...
  MFRC522_NTAG424DNA::DNA_CommMode commModeToSet;
  //commModeToSet = MFRC522_NTAG424DNA::DNA_COMMMODE_PLAIN;
  commModeToSet = MFRC522_NTAG424DNA::DNA_COMMMODE_MAC;
  //commModeToSet = MFRC522_NTAG424DNA::DNA_COMMMODE_FULL;
  // Access can be: 0, 1, 2, 3, 4, 0xE (free), 0xF (no access or RFU)
  byte readAccessToSet = 0xE;
  byte writeAccessToSet = 0;
  byte readWriteAccessToSet = 0;
  byte changeAccessToSet = 0;
  file = MFRC522_NTAG424DNA::DNA_FILE_CC; // default: PLAIN 0xE 0 0 0
  //file = MFRC522_NTAG424DNA::DNA_FILE_NDEF; // default: PLAIN 0xE 0xE 0xE 0
  //file = MFRC522_NTAG424DNA::DNA_FILE_PROPRIETARY; // default: FULL 2 3 3 0
  byte SDMEnabled = false; // DO NOT CHANGE, SDM IS NOT SUPPORTED!
  dna_statusCode = ntag.DNA_Full_ChangeFileSettings(file, commModeToSet, readAccessToSet, writeAccessToSet, readWriteAccessToSet, changeAccessToSet, SDMEnabled);
  if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_OK){
    Serial.print(F("Full_ChangeFileSettings STATUS NOT OK: "));
    Serial.println(dna_statusCode);
    if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_TIMEOUT)
      // Errors other than timeout appear often when slowly approaching a card to the reader.
      // When it happens, the card has to be deselected and woken up.
    ...
    Serial.println(F("Changed CommMode and R/W/RW/C Access successfully."));
  }
