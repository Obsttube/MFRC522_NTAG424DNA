...
void loop() {
  ...
  dna_statusCode = ntag.DNA_Plain_GetFileSettings_AccessRights(file, &readAccess, &writeAccess, &readWriteAccess, &changeAccess);
  if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_OK){
    Serial.print(F("Plain_GetFileSettings_AccessRights STATUS NOT OK: "));
    Serial.println(dna_statusCode);
    if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_TIMEOUT)
      // Errors other than timeout appear often when slowly approaching a card to the reader.
      // When it happens, the card has to be deselected and woken up.
    ...
    Serial.print(F("ReadAccess: "));
    Serial.print(readAccess, HEX);
    Serial.print(F("\nWriteAccess: "));
    Serial.print(writeAccess, HEX);
    Serial.print(F("\nReadWriteAccess: "));
    Serial.print(readWriteAccess, HEX);
    Serial.print(F("\nChangeAccess: "));
    Serial.print(changeAccess, HEX);
    Serial.println();
  }
