...
void loop() {
  ...
  byte keyNumber = 2; // key2 required to read the PROPRIETARY file
  ...
  // CC and NDEF files by default require CommMode Plain to read (see the Plain_ReadData example).
  MFRC522_NTAG424DNA::DNA_File file = MFRC522_NTAG424DNA::DNA_FILE_PROPRIETARY;
  // WARNING! Reading 128 B works on Arduino Uno, but setting the size of backData to more than ~220 fills the whole SRAM when using Authentication and Full CommMode
  // It works well on some variants of ESP8266, eg. the ESP-12E, ESP-12F or ESP-12S modules, which hava much more SRAM than Arduino Uno.
  uint16_t lengthToRead = 128; // set to 256 on ESP8266 when reading NDEF file
  byte* backData  = new byte[lengthToRead];
  memset(backData, 0, lengthToRead);
  uint16_t backLen = lengthToRead;
  byte offset = 0;
  dna_statusCode = ntag.DNA_Full_ReadData(file, lengthToRead, offset, backData, &backLen);
  if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_OK){
    Serial.print(F("Full_ReadData STATUS NOT OK: "));
    Serial.println(dna_statusCode);
    if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_TIMEOUT)
      // Errors other than timeout appear often when slowly approaching a card to the reader.
      // When it happens, the card has to be deselected and woken up.
