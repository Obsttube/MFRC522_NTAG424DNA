...
void loop() {
  ...
  bool FailedCtrEnabled = false;
  uint16_t TotFailCtrLimit = 0xFFFF; // When disabling, this value is ignored. default: 1000
  uint16_t TotFailCtrDecr = 0xFFFF; // When disabling, this value is ignored. default: 10
  dna_statusCode = ntag.DNA_Full_SetConfiguration_FailedCtrOption(FailedCtrEnabled, TotFailCtrLimit, TotFailCtrDecr);
  if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_OK){
    Serial.print(F("Full_SetConfiguration_FailedCtrOption STATUS NOT OK: "));
    Serial.println(dna_statusCode);
    if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_TIMEOUT)
      // Errors other than timeout appear often when slowly approaching a card to the reader.
      // When it happens, the card has to be deselected and woken up.
    ...
    Serial.println(F("Changed FailedCtrOption successfully."));
  }
