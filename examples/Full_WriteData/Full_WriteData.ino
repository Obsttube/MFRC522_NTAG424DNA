...
void loop() {
  ...
  byte keyNumber = 3; // key3 required to write to the PROPRIETARY file
  ...
  // CC and NDEF files by default require CommMode Plain to write (see the Plain_WriteData example).
  MFRC522_NTAG424DNA::DNA_File file = MFRC522_NTAG424DNA::DNA_FILE_PROPRIETARY;
  uint16_t lengthToWrite = 128;
  byte* sendData  = new byte[lengthToWrite];
  for (uint16_t i = 0; i < lengthToWrite; i++)
    sendData[i] = i;
  byte offset = 0;
  dna_statusCode = ntag.DNA_Full_WriteData(file, lengthToWrite, offset, sendData);
  if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_OK){
    Serial.print(F("Full_WriteData STATUS NOT OK: "));
    Serial.println(dna_statusCode);
    if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_TIMEOUT)
      // Errors other than timeout appear often when slowly approaching a card to the reader.
      // When it happens, the card has to be deselected and woken up.
    ...
    Serial.println(F("Wrote to file successfully."));
  }
  delete[] sendData;
