...
void loop() {
  ...
  // CC file by default requires key0 to write (see the Mac_WriteData example).
  // PROPRIETARY file by default requires key3 and CommMode FULL to write (see the Full_WriteData example).
  uint16_t lengthToWrite = 256;
  byte* sendData  = new byte[lengthToWrite];
  for (uint16_t i = 0; i < lengthToWrite; i++)
    sendData[i] = i;
  byte offset = 0;
  dna_statusCode = ntag.DNA_Plain_ISOUpdateBinary(file, lengthToWrite, offset, sendData);
  if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_OK){
    Serial.print(F("Plain_ISOUpdateBinary STATUS NOT OK: "));
    Serial.println(dna_statusCode);
    if (dna_statusCode != MFRC522_NTAG424DNA::DNA_STATUS_TIMEOUT)
      // Errors other than timeout appear often when slowly approaching a card to the reader.
      // When it happens, the card has to be deselected and woken up.
